...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for _ in range(options["playlists"]):
            ...
            playlist = PlaylistFactory(user=user)
        ...
        self.stdout.write(f"Creating {options['vocal-profiles']} vocal profiles...")
        vocal_profiles = []
        artists_sample = random.sample(artists, min(options["vocal-profiles"], len(artists)))
        for artist in artists_sample:
            vocal_profiles.append(VocalProfileFactory(artist=artist))
